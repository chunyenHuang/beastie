...
const dayScheduleComponent = {
    ...
    controller: /* @ngInject */ class DayScheduleController {
        static get $inject() {
            return [
                '$log', '$timeout', '$document', '$stateParams'
            ];
        }
        constructor(
            $log, $timeout, $document, $stateParams
        ...
        getPosition(scheduleAt){
            ...
            let startAt = scheduleAt.getHours() - this.officeHours.start;
            startAt = (startAt<0)?0:startAt;
            const left = startAt * this.grid.x;
        ...
        setDaySchedule(){
            this.dayTimes = [];
            for (var i = this.officeHours.start; i <= this.officeHours.end; i+=0.5) {
