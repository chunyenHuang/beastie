import template from './servicesDialog.html';
import './servicesDialog.styl';
...
class servicesDialog {
    ...
    constructor($document, $mdDialog) {
        ...
        if (!$document[0].getElementById('services-dialog')) {
            const div = $document[0].createElement('div');
            div.setAttribute('id', 'services-dialog');
            $document[0].body.appendChild(div);
        }
        const showDialog = (locals) => {
            return $mdDialog.show(
                this.dialog(locals,
                    $document[0].getElementById('services-dialog')
                )
            );
        };
        return showDialog;
    ...
    dialog(locals, parent) {
        return {
            ...
            parent,
            template,
            controller: /* @ngInject */ class servicesDialogController {
                static get $inject() {
                    return [
